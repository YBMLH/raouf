...
(function () {
  ...
  function buildThumb(page, position) {
    ...
    // Clicking the cell toggles selection (unless a touch-drag just ended).
    cell.addEventListener("click", () => {
      if (suppressNextClick) { suppressNextClick = false; return; }
      toggleSelect(page.id);
    });

    // Drag-to-reorder handlers (mouse + touch).
    attachDragHandlers(cell, page.id);
    attachTouchReorder(cell, page.id);
    return cell;
  }
  ...
  /* Touch reorder (phones/tablets): HTML5 drag events never fire for touch,
   * so we implement it directly. LONG-PRESS a thumbnail (~0.4s) to "lift" it,
   * slide your finger over another page, release to drop it there.
   * A short tap still just selects the page, and normal swipes still scroll. */
  let suppressNextClick = false;
  function attachTouchReorder(cell, id) {
    let timer = null;
    let lifted = false;
    ...
    cell.addEventListener("touchstart", () => {
      timer = setTimeout(() => {
        lifted = true;
        cell.classList.add("touch-lift");
        if (navigator.vibrate) navigator.vibrate(30);   // small haptic cue
      }, 400);
    }, { passive: true });

    cell.addEventListener("touchmove", (e) => {
      if (!lifted) { clearTimeout(timer); return; }     // finger moved early → it's a scroll
      e.preventDefault();                               // we're dragging: stop the page scrolling
      const t = e.touches[0];
      // Look through the whole element stack at the finger position: fixed
      // overlays (like the export bar) can sit on top of the grid, and the
      // thumbnail we want may be underneath them.
      let target = null;
      for (const el of document.elementsFromPoint(t.clientX, t.clientY)) {
        const thumb = el.closest && el.closest(".page-thumb");
        if (thumb) { target = thumb; break; }
      }
      clearTargets();
      if (target && Number(target.dataset.id) !== id) target.classList.add("drop-target");
    }, { passive: false });
    ...
    const finish = () => {
      ...
      if (!lifted) return;
      lifted = false;
      // The click generated by lifting the finger isn't a "select" — swallow
      // it. It fires within milliseconds (if at all: re-rendering the grid can
      // drop it), so expire the flag right after instead of letting a stale
      // flag eat the user's next real tap.
      suppressNextClick = true;
      setTimeout(() => { suppressNextClick = false; }, 350);
      cell.classList.remove("touch-lift");
      const target = els.pageGrid.querySelector(".drop-target");
      clearTargets();
      if (target) reorder(id, Number(target.dataset.id));
    };
